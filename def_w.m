function M = def_w(f_ck, h, w, o_L, n_L, d_L)

%--------------------------------------------------------------------------
% This function is defined be the user and converts the input to a
...
% LOAD
    ...
    % Transverse line load (from wind, ect.)
...
% ASSUMPTIONS
    ...
    % (Assumed to be equal vertical spacing and adequate, which is standard
    % pratice and easier to fabricate - Per)
    M.s_T = M.w/M.n_L(1);                       %[m]
...
% VOLUMES
    ...
    % (Two rows with equal spacing across the wall's height)
    M.V_sT = (2 * M.L/M.s_T * pi * (M.o_T/2)^2) * M.w;          %[m^3]
