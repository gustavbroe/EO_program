...
% Limits for wall thickness and longitunal reinforcement diameter
o_L_min = 4;                            o_L_max = 20;   %[mm]
w_min = max(150, 2*(o_L_max+15+6));     w_max = 300;    %[mm]
...
% Adding plot title
title('', ...
    sprintf(['(L = %.f [mm], w = %.f [mm] f_c_k = %.f, ' ...
    'rho_s = %.f [kg/m^3])'], M.L*10^3, M.w*10^3, M.f_ck*10^(-6), ...
    MI.(EPD_s).rho));
...
% Adding plot title
title('', ...
    sprintf(['(o_L = %.2f [mm], L = %.f [mm], w = %.f [mm] f_c_k = %.f, ' ...
    'rho_s = %.f [kg/m^3])'], ...
    oo(size(PP,2)/2, 1), M.L*10^3, M.w*10^3, M.f_ck*10^(-6), ...
    MI.(EPD_s).rho));
...
path_folder = '.\results\Pricing\Wall';
