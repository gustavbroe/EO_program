...
for x = 1:size(o_L, 2)
    ...
    for y = 1:size(s, 2)
        ...
        % Definitions
        M = def_c(f_ck, s(y), s(y), repmat(o_L(x), 1, size(n_L, 2)), n_L, []);
...
surf(ss, oo, MM ,"EdgeColor", "k", "EdgeAlpha", 0.5, ...
    "HandleVisibility", "off");

% Plotting first order moment, M_0Ed
if show_planes == 1
    surf(ss, oo, repmat(M.M_0Ed,np,np) ,"EdgeColor", "k", ...
...
for k = 1:3
    ...
    % Plotting the planes
    if show_planes == 1
        surf(ss, oo, d{k} ,"EdgeColor", "k", "EdgeAlpha", 0.2, ...
...
plot3(ss(vcs), oo(vcs), MM(vcs), "MarkerSize", 16, "Marker", ".", ...
...
surf(ss, oo, GG ,"EdgeColor", "k", "EdgeAlpha", 0.5, ...
    "HandleVisibility", "off");
...
axis([s_min s_max o_L_min o_L_max])
...
for i = ii
    ...
    if con_price == 1
    figure(p3); hold on;
    plot3(ss(vr{i}), oo(vr{i}), PP(vr{i}), "MarkerSize", 16, "Marker", ...
    ...
    figure(p2); hold on;
    plot3(ss(vr{i}), oo(vr{i}), GG(vr{i}), "MarkerSize", 16, "Marker", ...
        ".", "Color", c{i}, "LineStyle", "none", "DisplayName", tx{i});
    figure(p2); hold off;   
    ...
    if control_gwp == 1
        ...
        while control_gwp == 1
            ...
            % Calling function
            fig_IC = input_control(0.03, def_c(f_ck, ss(row,col), ...
                ss(row,col), repmat(oo(row,col), 1, size(n_L, 2)), n_L, []));
...
%--------------------------------------------------------------------------
%% Outputting cheapest column and plotting it (all plots)

if con_price == 1

% Printing header
fprintf('\n\nThe cheapest column for each method is: \n\n')
...
path_folder = '.\results\Column\Three_dimensional';
